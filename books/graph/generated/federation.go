	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		case "Assignment":
			...
			case "findAssignmentByStudentID":
				id0, err := ec.unmarshalNString2string(ctx, rep["studentId"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findAssignmentByStudentID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindAssignmentByStudentID(ctx, id0)
				if err != nil {
		...
		case "Student":
			resolverName, err := entityResolverNameForStudent(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Student": %w`, err)
			}
			switch resolverName {
			...
			case "findStudentByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findStudentByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindStudentByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Student": %w`, err)
				}
...
func entityResolverNameForAssignment(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		...
		_ = val
		m = rep
		if _, ok = m["studentId"]; !ok {
			break
		}
		return "findAssignmentByStudentID", nil
	}
	return "", fmt.Errorf("%w for Assignment", ErrTypeNotFound)
}

func entityResolverNameForStudent(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
			m   map[string]interface{}
			val interface{}
		...
		return "findStudentByID", nil
	}
	return "", fmt.Errorf("%w for Student", ErrTypeNotFound)
}
